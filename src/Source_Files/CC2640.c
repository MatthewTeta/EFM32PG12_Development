...
static uint32_t        cc2640_rx_byte    = 0;
...
void CC2640_open(LEUART_TypeDef *leuart_x, scheduler_event_t rx_cb_event) {
  ...
  leuart_open_t o = {
      .init =
          {
              .baudrate = CC2640_BAUD,
              .databits = 8,
              .enable   = leuartEnable,
              .parity   = LEUART_CTRL_PARITY_EVEN,
              .refFreq  = 0,
...
void CC2640_request_help(void) {
  static uint8_t cmd_help[] = "AT+HELP?";
  CC2640_tx_buff(cmd_help, sizeof(cmd_help) / sizeof(uint8_t), 0);
}
