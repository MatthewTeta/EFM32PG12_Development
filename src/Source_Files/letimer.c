...
//  LETIMER_Reset(letimer);
	...
	period_active_cnt = period_cnt * app_letimer_struct->active_period / app_letimer_struct->period;
	 ...
	 */
	letimer->ROUTELOC0 =
	    ((app_letimer_struct->out_pin_route0 & 0x3F) << 0) |
	    ((app_letimer_struct->out_pin_route1 & 0x3F) << 8);
	letimer->ROUTEPEN  =
	    ((app_letimer_struct->out_pin_0_en & 0x01) << 0) |
...
//	letimer->ROUTEPEN = 3;
  ...
  uint32_t flag_mask =  (LETIMER_IF_COMP0 & app_letimer_struct->comp0_isr_enable<<31>>31) |
  ...
  /* Block the energy mode which will inhibit the peripheral's operation if the timer is running */
 ...
 *
 * @note
 *   The timer will have to synchronize its' clock before it will output (blocking)
 * @param letimer
...
void letimer_start(LETIMER_TypeDef *letimer, bool enable){
  // Determine if the function inputs will cause a transition in the state of the timer:
